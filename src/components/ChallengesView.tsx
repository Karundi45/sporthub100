...
const initialLeaders: Leader[] = [
  { id: "1", name: "Alex W.", score: 95.2, avatar: "A", rankChange: "same" },
  { id: "2", name: "Samira K.", score: 88.4, avatar: "S", rankChange: "same" },
  { id: "3", name: "Mike T.", score: 82.1, avatar: "M", rankChange: "same" },
  { id: "4", name: "Guest User", score: 65.2, avatar: "G", rankChange: "same" },
  { id: "5", name: "Elena R.", score: 60.5, avatar: "E", rankChange: "same" },
];
...
const initialGroups: PrivateGroup[] = [
  {
    ...
    challengeType: "Most distance in a week",
    members: [
      { name: "Alex W.", avatar: "A", score: 15.2 },
      { name: "Guest User", avatar: "G", score: 12.5 },
      { name: "Samira K.", avatar: "S", score: 8.4 }
    ],
    inviteCode: "WKD-RUN"
  }
];
...
export function ChallengesView() {
  ...
  const [groups, setGroups] = useState<PrivateGroup[]>(initialGroups);
  ...
  const handleCreateGroup = () => {
    ...
    const newGroup: PrivateGroup = {
      ...
      members: [{ name: "Guest User", avatar: "G", score: 0 }],
  ...
  const challenges = [
    {
      id: "1",
      title: "100KM Monthly Run",
      description: "Run 100km total this month to earn the Endurance badge.",
      progress: 65.2,
      target: 100,
      unit: "KM",
      daysLeft: 12,
    ...
    },
    {
      ...
      description: "Climb a total of 8,848m in elevation over 30 days.",
      progress: 3200,
      target: 8848,
      unit: "M",
      daysLeft: 18,
    ...
    },
    {
      ...
      description: "Complete 3 intense workouts this weekend.",
      progress: 1,
      target: 3,
      unit: "Workouts",
      daysLeft: 2,
        ...
        {activeTab === "challenges" ? (
          ...
          >
            {challenges.map((c) => {
              ...
              return (
                <div key={c.id} className="bg-brand-surface border border-brand-border rounded-md p-5 hover:border-brand-accent/30 transition-all flex flex-col justify-between shadow-sm">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 bg-[#1e1e1e] border border-brand-border rounded flex items-center justify-center shadow-inner">
                        <Icon className="w-5 h-5 text-brand-text-primary" />
                      </div>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-brand-text-secondary bg-[#1e1e1e] border border-brand-border px-2.5 py-0.5 rounded">
                        {c.daysLeft} Days Left
                      </span>
                    </div>
                    <h3 className="text-base font-semibold text-brand-text-primary mb-1.5">{c.title}</h3>
                    <p className="text-[13px] text-brand-text-secondary mb-6">{c.description}</p>
                  </div>

                  <div>
                    <div className="flex justify-between items-end mb-1.5">
                      <span className="text-[13px] font-semibold text-brand-accent">{c.progress} <span className="text-[11px] text-brand-text-secondary font-medium">{c.unit}</span></span>
                      <span className="text-[12px] text-brand-text-secondary font-medium">Goal: {c.target} {c.unit}</span>
                    </div>
                    <div className="h-1.5 w-full bg-[#111] rounded-full overflow-hidden border border-[rgba(255,255,255,0.05)]">
                      <motion.div 
                        initial={{ width: 0 }}
                    ...
                    <div className="mt-4 flex items-center gap-2 text-[11px] text-brand-text-secondary font-medium">
                      <span className="flex -space-x-1">
                        {[1, 2, 3].map((i) => (
                          <div key={i} className="w-4 h-4 rounded-full border border-brand-surface bg-[#444]" />
                        ))}
                      </span>
