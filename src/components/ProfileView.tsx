...
import { Settings, Edit2, Users, Activity, Target, Trophy, Calendar, Dumbbell, Flame, CheckCircle2, ChevronRight, BarChart3, Clock, TrendingUp, MapPin, UserPlus, X, Save, Search, Camera, MessageSquare, Trash2, Send, Image as ImageIcon, Hash, AtSign } from 'lucide-react';
...
export function ProfileView({ currentUser }: { currentUser: any }) {
  ...
  const [profileData, setProfileData] = useState({
    name: currentUser?.settings?.name || "Fitness Enthusiast",
    username: currentUser?.username || "user",
    location: currentUser?.settings?.location || "Planet Earth",
    bio: currentUser?.settings?.bio || "Marathon runner and fitness enthusiast.",
    email: currentUser?.email || "",
    notifications: true,
    privateProfile: false,
    profilePic: currentUser?.profilePic || "https://i.pravatar.cc/150?u=sarah"
  });
  ...
  React.useEffect(() => {
    if (currentUser) {
      setProfileData(prev => ({
        ...
        name: currentUser.settings?.name || prev.name,
        bio: currentUser.settings?.bio || prev.bio,
        location: currentUser.settings?.location || prev.location
      }));
    }
  }, [currentUser]);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        if (globalSocket) {
           globalSocket.emit('update_profile', { username: currentUser.username, profilePic: data.url });
        }
      }
    } catch (err) {
      console.error(err);
  ...
  React.useEffect(() => {
    if (globalSocket) {
      const handleDashboardUpdate = (activities: any[]) => {
        setPosts(activities.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()));
        
        // Calculate weekly stats
        const weekStats = activities.reduce((acc, act) => {
          const date = new Date(act.time);
          const day = date.toLocaleDateString('en-US', { weekday: 'short' });
          if (!acc[day]) acc[day] = { day, calories: 0, activeMin: 0 };
          if (act.details?.calories) acc[day].calories += act.details.calories;
          if (act.details?.duration) acc[day].activeMin += act.details.duration;
          return acc;
        }, {} as any);
        setWeeklyData(Object.values(weekStats));
      };
      
      globalSocket.on('dashboard_data_update', handleDashboardUpdate);
      globalSocket.on('feed_update', (newAct) => {
        if (newAct.user === currentUser?.username) {
           setPosts(prev => [newAct, ...prev]);
        }
      });
      return () => {
        globalSocket.off('dashboard_data_update', handleDashboardUpdate);
        globalSocket.off('feed_update');
      };
    }
  }, [currentUser]);
      ...
      <div className="px-6 md:px-8 pt-8 pb-6 border-b border-[#2A2D3A]">
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#21D4B5] to-[#32ADE6] p-1 relative group cursor-pointer">
              <label htmlFor="avatar-upload" className="w-full h-full cursor-pointer relative block">
                <img src={profileData.profilePic} className="w-full h-full rounded-full border-2 border-[#1A1C23] object-cover bg-gray-800" alt="Profile" />
                <div className="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <Camera className="w-6 h-6 text-white" />
                </div>
              </label>
              <input type="file" id="avatar-upload" accept="image/*" className="hidden" onChange={handleAvatarUpload} disabled={uploadingImage} />
              {uploadingImage && <div className="absolute -inset-2 border-2 border-[#21D4B5] border-t-transparent rounded-full animate-spin pointer-events-none" />}
            </div>
            <div>
      ...
      <div className="flex-1 p-6 md:p-8 max-w-5xl mx-auto w-full">
        ...
        {activeTab === "history" && (
          <div className="space-y-4">
             ...
             {posts.filter(p => p.details || p.activity !== 'Post').map((workout, i) => (
               <div key={i} className="bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] p-4 flex items-center justify-between group hover:bg-[#2A2D3A]/20 transition-colors cursor-pointer">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                       <h4 className="text-white font-bold text-base">{workout.activity}</h4>
                       <p className="text-[#8E92A4] text-xs mb-1">{new Date(workout.time).toLocaleString()} • {workout.details?.duration || 0}m</p>
                       <div className="inline-block bg-[#1A1C23] px-2 py-0.5 rounded text-[10px] font-bold text-[#8E92A4]">
                         {workout.details?.distance || 0} mi • {workout.details?.calories || 0} kcal
                       </div>
                     </div>
                  </div>
        ...
        {activeTab === "badges" && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-4 bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] shadow-sm">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     <h4 className="text-base font-bold text-white mb-0.5">First 5K</h4>
                     <p className="text-xs text-[#8E92A4]">Completed your first 5K run</p>
                   </div>
                </div>
                <span className="text-xs font-bold text-[#8E92A4]">Apr 12, 2024</span>
              </div>

              <div className="flex items-center justify-between p-4 bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] shadow-sm">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-gradient-to-br from-[#FFB300] to-[#FF6B00] rounded-full flex items-center justify-center shadow-md">
                     <Flame className="w-6 h-6 text-white" />
                   </div>
                   <div>
                     <h4 className="text-base font-bold text-white mb-0.5">7-Day Streak</h4>
                     <p className="text-xs text-[#8E92A4]">Worked out for 7 days in a row</p>
                   </div>
                </div>
                <span className="text-xs font-bold text-[#8E92A4]">May 03, 2024</span>
              </div>

              <div className="flex items-center justify-between p-4 bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] shadow-sm">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-gradient-to-br from-[#9D74FF] to-[#814FFF] rounded-full flex items-center justify-center shadow-md">
                     <CheckCircle2 className="w-6 h-6 text-white" />
                   </div>
                   <div>
                     <h4 className="text-base font-bold text-white mb-0.5">Early Bird</h4>
                     <p className="text-xs text-[#8E92A4]">5 AM workout done</p>
                   </div>
                </div>
                <span className="text-xs font-bold text-[#8E92A4]">Jun 15, 2024</span>
              </div>

              <div className="flex items-center justify-between p-4 bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] shadow-sm opacity-50 grayscale hover:grayscale-0 cursor-not-allowed">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-[#2A2D3A] rounded-full flex items-center justify-center shadow-md">
                     <Trophy className="w-6 h-6 text-[#8E92A4]" />
                   </div>
                   <div>
                     <h4 className="text-base font-bold text-white mb-0.5">Century Ride</h4>
                  ...
                  <button 
                    onClick={() => {
                      if (newPostText.trim() || newPostPicture) {
                        ...
                        const newActivity = {
                          activity: newPostWorkoutType.trim() || 'Post',
                          metrics: newPostText,
                          caption: newPostText,
                          image: newPostPicture,
                        ...
                        if (globalSocket) {
                          globalSocket.emit("new_activity", newActivity);
                        } else {
                          // Fallback to queue if offline
                          const queue = JSON.parse(localStorage.getItem('offline_posts_queue') || '[]');
                          queue.push(newActivity);
                          localStorage.setItem('offline_posts_queue', JSON.stringify(queue));
                        }
              ...
              {posts.map(post => (
                <div key={post.id} className="bg-[#22252E] border border-[#2A2D3A]/50 rounded-[20px] p-5 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[#8E92A4] text-xs">{new Date(post.time).toLocaleString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      {editingPostId !== post.id && (
                        <>
                          <button 
                            onClick={() => { setEditingPostId(post.id); setEditingPostText(post.caption); }}
                  ...
                  {editingPostId === post.id ? (
                    ...
                    <>
                      <p className="text-[#E0E2E8] text-sm whitespace-pre-wrap">{post.caption}</p>
                      
                      {post.image && (
                        <div className="mt-3 rounded-xl overflow-hidden border border-[#2A2D3A]">
                      ...
                      {/* Workout Data */}
                      {post.workoutData && (
                        <div className="mt-3 bg-[#1A1C23] border border-[#2A2D3A] rounded-xl p-3 flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-[#2A2D3A] flex items-center justify-center">
                            <ActivityIcon className="w-5 h-5 text-[#21D4B5]" />
                          </div>
                          <div>
                            <p className="text-white text-sm font-bold">{post.activityName || "Workout"}</p>
