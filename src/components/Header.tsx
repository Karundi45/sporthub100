              ...
              {showProfileMenu && (
                ...
                >
                  <div className="p-4 border-b border-brand-border shrink-0 bg-brand-surface-light flex items-center gap-3">
                    <div className="w-12 h-12 bg-[#111] rounded-full flex items-center justify-center font-bold text-lg border border-brand-border overflow-hidden">
                      {user?.profilePic ? <img src={user.profilePic} className="w-full h-full object-cover" /> : (user?.settings?.name?.charAt(0) || user?.username?.charAt(0) || 'U')}
                    </div>
                    <div>
                      <h3 className="font-bold text-brand-text-primary">{user?.settings?.name || user?.username || "User"}</h3>
                      <p className="text-[11px] text-brand-text-secondary">Level {user?.stats?.level || 1} • {user?.stats?.xp || 0} XP</p>
                    </div>
                  </div>
