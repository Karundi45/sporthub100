...
import { getActivityData, saveActivityData, getPersonalBests, savePersonalBests } from "@/lib/db";
...
export function DashboardView() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        let cachedPBs = await getPersonalBests();
        if (!cachedPBs || cachedPBs.length === 0) {
          await savePersonalBests(DEFAULT_PERSONAL_BESTS);
          cachedPBs = DEFAULT_PERSONAL_BESTS;
        }
        setPersonalBests(cachedPBs);
      } catch (err) {
    ...
    if (globalSocket) {
      globalSocket.on('dashboard_data_update', (activities: any[]) => {
        // Aggregate activities into daily buckets (Mon-Sun)
        const dayOrder = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
        const newActivityData = [...DEFAULT_ACTIVITY_DATA]; // Start with defaults
        
        // Very basic mock aggregation logic for demonstration:
        activities.forEach(act => {
           const d = new Date(act.time);
           const dayName = dayOrder[d.getDay() === 0 ? 6 : d.getDay() - 1];
           const index = newActivityData.findIndex(x => x.day === dayName);
           if (index !== -1 && act.details) {
              newActivityData[index].calories += (act.details.calories || 0);
              newActivityData[index].distance += (act.details.distance || 0);
              newActivityData[index].minutes += (act.details.duration || 0);
              newActivityData[index].workouts += 1;
           }
        });
  ...
  // Real-time scrolling memory for HR telemetry
  useEffect(() => {
    let interval: any;
    if (heartRate !== null || isSimulatingHR) {
      interval = setInterval(() => {
        let currentBpm = heartRate;
        if (isSimulatingHR) {
          // Keep simulated HR fluctuating realistically around 125-145 representing steady cardio
          const base = 132;
          const fluctuation = Math.sin(Date.now() / 6000) * 12 + (Math.random() * 5 - 2.5);
          currentBpm = Math.round(base + fluctuation);
        ...
        if (currentBpm) {
          setHrHistory((prev) => {
            ...
            const updated = [...prev, { time: timeStr, bpm: currentBpm }];
  ...
  // Generate mock days for the calendar
  // Simulated Plan Progress
  const [planProgress, setPlanProgress] = useState({ completed: 1, total: 7, percentage: 14 });

  useEffect(() => {
    // Realtime simulated progress for the Plan Banner
    const timer = setInterval(() => {
      setPlanProgress(prev => {
        const nextPercentage = prev.percentage < 100 ? prev.percentage + 1 : 0;
        const nextCompleted = Math.max(1, Math.floor((nextPercentage / 100) * prev.total));
        return {
          ...prev,
