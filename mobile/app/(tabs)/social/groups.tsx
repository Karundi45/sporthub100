...
export default function GroupsScreen() {
  ...
  const renderGroup = ({ item }: { item: Group }) => {
    ...
    return (
      <TouchableOpacity style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border, borderWidth: 1 }]} onPress={() => handleGroupPress(item)}>
        <View style={styles.cardHeader}>
          <Text style={[styles.title, { color: theme.text }]}>{item.name}</Text>
          <View style={[styles.badge, { backgroundColor: isMember ? theme.success + '33' : theme.primary + '33' }]}>
            <Text style={[styles.badgeText, { color: isMember ? theme.success : theme.primary }]}>{isMember ? 'Member' : 'Join'}</Text>
          </View>
        </View>
        <Text style={[styles.description, { color: theme.textSecondary }]}>{item.description}</Text>
        <Text style={[styles.membersCount, { color: theme.textSecondary }]}>{item.members.length} Members</Text>
      </TouchableOpacity>
    );
  };
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={[styles.largeTitle, { color: theme.text }]}>Community</Text>
          <TouchableOpacity onPress={() => setIsCreating(!isCreating)}>
            <Text style={[styles.createText, { color: theme.primary }]}>{isCreating ? 'Cancel' : 'Create'}</Text>
          </TouchableOpacity>
        </View>

        {isCreating && (
          <View style={[styles.createContainer, { backgroundColor: theme.surface }]}>
            <TextInput style={[styles.input, { backgroundColor: theme.inputBackground, color: theme.text }]} placeholder="Group Name" placeholderTextColor={theme.textSecondary} value={newGroupName} onChangeText={setNewGroupName} />
            <TextInput style={[styles.input, { backgroundColor: theme.inputBackground, color: theme.text }]} placeholder="Description" placeholderTextColor={theme.textSecondary} value={newGroupDesc} onChangeText={setNewGroupDesc} />
            <TouchableOpacity style={[styles.submitButton, { backgroundColor: theme.success }]} onPress={() => createMutation.mutate()}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={[styles.emptyText, { color: theme.textSecondary }]}>No groups available.</Text>}
        />
      </View>
    </SafeAreaView>
  );
